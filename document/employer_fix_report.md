...
- **Hướng sửa:** Tạo bottom sheet chọn phương thức thanh toán hoặc trang nạp tiền riêng
...
- **Hướng sửa:** Tạo form rút tiền với xác nhận OTP/mật khẩu
...
- **Hướng sửa:** Đổi thành `navigate({ to: '/employer/post-job', search: { editJobId: job.id } })`
...
- **Hướng sửa:** Map `job.genderPreference` → "Nam" | "Nữ" | "Cả hai"
...
- **Hướng sửa:** `onViewAll={() => navigate({ to: '/employer/job-list' })}`
...
- **Hướng sửa:** Query attendance records, hiển thị danh sách NV với thời gian check-in/out, GPS
...
- **Hướng sửa:** Đổi filter sang `candidateName.toLowerCase().includes(search)`

---

## 🟡 MEDIUM – Cần cải thiện

### 13. Error handling dùng alert() thay vì toast
...
### 14. Bell button trên Profile không có handler
...
### 15. Achievement badges hardcoded
- **File:** `routes/employer/profile/index.tsx` dòng 251
- **Vấn đề:** Dùng `PREDEFINED_ACHIEVEMENTS` constant, không lấy từ user data
- **Hướng sửa:** Query achievements đã unlock từ Firestore user profile

### 16. Dark mode toggle không hoạt động thực
...
### 17. Language selector không hoạt động thực
...
### 18. Notification toggles không persist
- **File:** `routes/employer/profile/settings/index.tsx` dòng 26-27
- **Vấn đề:** Push/Email toggles chỉ là local state, reload mất
- **Hướng sửa:** Lưu preferences vào Firestore user document

### 19. Shift status hardcoded "Đang mở"
...
### 20. Categories post-job hardcode 4 mục
- **File:** `routes/employer/post-job.tsx` dòng 43
- **Vấn đề:** Chỉ có `['F&B Service', 'Retail', 'Delivery', 'Event Helper']`
- **Hướng sửa:** Lấy danh mục từ Firestore collection `categories` hoặc mở rộng list

### 21. Job List thiếu max-w container trên desktop
...
### 23. Profile Edit thiếu validation form
...
### 24. Emoji checkmark thay vì SVG icon
