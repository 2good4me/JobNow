  ...
  return (
    <div className="min-h-[100dvh] bg-slate-50 pb-24 font-sans text-slate-800">
      ...
      <div className="px-4 -mt-4 relative z-20">
        ...
        {!user && (
          <div className="mb-6 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800 flex items-start gap-3">
            <Lock className="w-5 h-5 shrink-0 mt-0.5" />
            <div>
              <p className="font-bold">Đăng nhập để ứng tuyển, lưu việc, và theo dõi nhà tuyển dụng.</p>
              <button
                type="button"
        ...
        <div
          className="bg-white shadow-sm border border-slate-100 rounded-2xl p-4 flex items-center gap-4 mb-8 cursor-pointer hover:bg-slate-50 transition-colors"
          onClick={() => {
            if (!user) {
        ...
        >
          ...
          <div className="flex-1">
            ...
            <p className="text-slate-500 text-[12px]">
              {user ? 'Hoạt động tích cực' : 'Đăng nhập để xem hồ sơ và thông tin liên hệ đầy đủ'}
            </p>
          </div>
      ...
      <div className="fixed bottom-[72px] inset-x-0 bg-white border-t border-slate-200 p-4 pb-safe flex gap-3 z-[60]">
        ...
        >
          {applyMutation.isPending ? 'ĐANG XỬ LÝ...' : hasApplied ? 'ĐÃ ỨNG TUYỂN' : selectedShiftRemaining <= 0 ? 'CA LÀM ĐÃ HẾT CHỖ' : user ? 'ỨNG TUYỂN NGAY' : 'ĐĂNG NHẬP ĐỂ ỨNG TUYỂN'}
        </button>
      </div>
