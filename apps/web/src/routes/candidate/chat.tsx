...
function CandidateChatRoute() {
  ...
  const canSendMessage = selectedConversation?.chatPermission === 'TWO_WAY';
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Error is shown by disabled state + hint below.
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24">
      <div className="max-w-lg mx-auto px-4 pt-5 space-y-4">
        <h1 className="text-xl font-bold text-slate-900">Tin nhắn công việc</h1>
        ...
        <section className="rounded-2xl bg-white border border-slate-100 shadow-sm">
          <div className="px-4 py-3 border-b border-slate-100">
            <p className="text-sm font-semibold text-slate-700">Hội thoại của bạn</p>
          </div>
          ...
          {isLoadingConversations ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-xs text-slate-400 mt-1">Ứng tuyển và chờ nhà tuyển dụng mở chat để trao đổi.</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-100">
              {conversations.map((conversation) => {
                const isActive = conversation.id === selectedConversationId;
                const unreadCount = getConversationUnreadCount(conversation, 'CANDIDATE');
          ...
          {!selectedConversationId ? (
            ...
            <>
              ...
              <div className="border-t border-slate-100 p-3 bg-white">
                {!canSendMessage ? (
                  <p className="text-xs text-amber-600">Bạn chỉ có thể gửi tin nhắn sau khi đơn được duyệt hoặc nhà tuyển dụng mở chat.</p>
                ) : (
