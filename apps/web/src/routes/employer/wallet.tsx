...
function EmployerWalletPage() {
  ...
  const isIncome = (type: string) => ['DEPOSIT', 'REFUND', 'VNPAY_TOPUP'].includes(type.toUpperCase());
  const isExpense = (type: string) => ['PAYMENT', 'WITHDRAW', 'BOOST', 'PREMIUM', 'BOOST_JOB'].includes(type.toUpperCase());

  const stats = transactions.reduce((acc, tx) => {
    if (tx.status !== 'COMPLETED') return acc;
    if (timeFilter === 'MONTH' && tx.createdAt < startOfMonth) return acc;
  ...
  const filteredTransactions = transactions.filter((tx) => {
    if (activeFilter === 'INCOME') return isIncome(tx.type);
    if (activeFilter === 'EXPENSE') return isExpense(tx.type);
    return true;
  });
      ...
      <div className="mx-auto w-full max-w-lg px-5 pt-12">
        ...
        <div className="flex items-center justify-between mb-6">
          <button
            ...
            className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-slate-600 shadow-sm border border-slate-100 active:scale-95 transition-all"
          >
            <ArrowLeft className="w-5 h-5" />
          </button>
          <h1 className="text-lg font-black text-slate-900 tracking-tight">Ví nhà tuyển dụng</h1>
          <button
        ...
        {/* ── Balance Card (Matching Candidate) ── */}
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-2.5 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-sm">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] text-white/60">Current Balance</span>
                </div>
              ...
              <div className="mt-8 grid grid-cols-2 gap-3">
                ...
                >
                  ...
                  <PlusCircle className="relative h-4 w-4 text-blue-600" />
                  <span className="relative">Nạp tiền</span>
                </button>
                <button
                  onClick={() => setShowWithdrawSheet(true)}
                  className="flex items-center justify-center gap-2 rounded-2xl bg-white/10 border border-white/10 px-4 py-4 text-sm font-bold text-white backdrop-blur-md transition-all active:scale-95 hover:bg-white/20"
                >
        ...
        {/* ── Stats Grid (Matching Candidate Style) ── */}
        <section className="mt-5 grid grid-cols-3 gap-3">
          <div className="rounded-[24px] bg-white p-4 shadow-sm border border-slate-100 flex flex-col items-center">
            ...
            <p className="mt-1 text-[13px] font-black text-emerald-600">
              {stats.income > 0 ? '+' : ''}{formatAmount(stats.income)}
            </p>
          </div>
          <div className="rounded-[24px] bg-white p-4 shadow-sm border border-slate-100 flex flex-col items-center">
            ...
            <p className="text-[9px] font-bold uppercase tracking-[0.1em] text-slate-400">Chi tiêu</p>
            <p className="mt-1 text-[13px] font-black text-rose-600">-{formatAmount(stats.expense)}</p>
          </div>
          <div className="rounded-[24px] bg-white p-4 shadow-sm border border-slate-100 flex flex-col items-center">
        ...
        <section id="wallet-transactions" className="mt-7 space-y-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {[
              ...
              <button
                key={chip.key}
                onClick={() => setActiveFilter(chip.key as 'ALL' | 'INCOME' | 'EXPENSE')}
            ...
            {isLoadingTx ? (
              ...
              <div className="rounded-[28px] bg-white px-6 py-10 text-center shadow-[0_12px_32px_rgba(15,23,42,0.06)]">
                ...
                <p className="mt-2 text-sm leading-relaxed text-slate-500">
                  Các giao dịch nạp tiền, rút tiền hoặc thanh toán sẽ hiển thị tại đây.
                </p>
              </div>
            ) : (
              filteredTransactions.map((tx) => {
                const isIncomeTx = isIncome(tx.type) || tx.amount > 0;
                  ...
                  >
                    <div
                      className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl transition-colors ${
                        isIncomeTx ? 'bg-emerald-50 text-emerald-600 group-hover:bg-emerald-100' : 'bg-rose-50 text-rose-600 group-hover:bg-rose-100'
                      }`}
                    >
                      {isIncomeTx ? <ArrowDownLeft className="h-6 w-6" /> : <ArrowUpRight className="h-6 w-6" />}
                    </div>
                    ...
                    <div className="text-right">
                      <p className={`text-[16px] font-black tracking-tight ${isIncomeTx ? 'text-emerald-600' : 'text-slate-900'}`}>
                        {isIncomeTx ? '+' : '-'}{formatAmount(Math.abs(tx.amount))}
                      </p>
                    </div>
                  </div>
                );
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm leading-relaxed text-white/65">
                Đội ngũ CSKH sẽ hỗ trợ khi giao dịch gặp vấn đề hoặc cần xác minh thêm.
              </p>
            </div>
            <button
