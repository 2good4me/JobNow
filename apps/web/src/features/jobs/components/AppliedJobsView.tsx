...
export function AppliedJobsView() {
  ...
  const filters: { label: string; value: FilterStatus }[] = [
    ...
    { label: `Chờ duyệt (${applications.filter(a => a.status === 'NEW' || a.status === 'PENDING').length})`, value: 'PENDING' },
    { label: `Đã nhận (${applications.filter(a => a.status === 'APPROVED').length})`, value: 'APPROVED' },
    { label: `Từ chối (${applications.filter(a => a.status === 'REJECTED').length})`, value: 'REJECTED' },
  ];

  return (
    <div className="space-y-4">
      {/* Filters (Tonal pills from Stitch design) */}
          ...
          return (
            <button
              ...
              className={`px-4 py-2 rounded-2xl text-[14px] font-bold transition-colors whitespace-nowrap flex-shrink-0 ${
                isActive
                  ? 'bg-[#006399] text-white shadow-sm'
      ...
      <div className="space-y-3 pt-2">
        {isLoading ? (
          ...
          <div className="text-center py-16 px-4 bg-white rounded-[24px] border border-[#E2E2E9] shadow-sm">
            ...
            <h2 className="text-lg font-headline font-bold text-[#191C1E] mb-2">Chưa có đơn nào</h2>
